...
#include<ABT/frwanalysis.h>
#include<ABT/frandomgen.h>
#include<ABT/charter.h>
...
int main(int argc,char **argv)
{
    ...
    button1 = gtk_button_new_with_mnemonic( "Random generator" );
    ...
    gtk_window_set_title(GTK_WINDOW(window), "ABT 0.7.0");
...
void randomfilegen()
{
	...
	else if(g_ascii_strcasecmp(ans,"searching")==0)
	{
		printf("random-generation-searching\n");
		frandomgen_main(1);
	}
...
void load_file(GtkWidget *button,gpointer* win)
{
    ...
    else if(response==GTK_RESPONSE_OK && g_ascii_strcasecmp(ans,"searching")==0)
    {
        g_print("%s\n",gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog)));
        g_print("%s %d\n",ans, g_ascii_strcasecmp(ans,"searching"));
        fileread_search(gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog)));
    }
    else if(response==GTK_RESPONSE_OK && g_ascii_strcasecmp(ans,"diskscheduling")==0)
    {
        g_print("%s\n",gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog)));
        g_print("%s %d\n",ans, g_ascii_strcasecmp(ans,"diskscheduling"));
        fileread_disk(gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog)));
    }
    else 
    {
        g_print("canceled\n");
